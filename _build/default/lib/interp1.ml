...
let exec (p : Ast.Program.t) : unit =
  let main_function_name = "main" in
  match List.find_opt (fun fd -> Ast.Id.compare fd.Ast.Function.name main_function_name = 0) p.Ast.Program.functions with
  | Some main_function ->
    (* Set up initial environment and execute the main function *)
    let initial_environment = [] (* Assuming an empty initial environment, replace with actual initial environment setup if necessary *) in
    let initial_frame = [] (* Similarly, set up the initial frame if your semantics require it *) in
    (* Assuming exec_function is a function you implement to execute functions given a name, environment, and frame *)
    ignore (exec_function main_function_name initial_environment initial_frame);
  | None ->
